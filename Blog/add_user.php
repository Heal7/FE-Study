...
?>
<?php
	if(isset($POST["username"])&&isset($_POST["password"])&&isset($_POST["confirm"]))
	{
		$user = $_POST["username"];
		$psw = $_POST["password"];
		...
		if($user == "" || $psw == "" || $psw_confirm == ""){
			echo "<script>alert('请确认信息完整！');history.go(-1)";
			exit;
		}
		else{
			if($psw == $psw_confirm){
				...
				if($num){//如果已经存在该用户
					...
					$sql_insert = "insert into user(username,password) value ('$_POST[username]','$psw')";
